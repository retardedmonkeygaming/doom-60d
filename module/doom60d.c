...
struct doom_raw_trace_entry {
    uint32_t ms; uint32_t type; uint32_t param; uint32_t arg; uint32_t obj;
};
...
// --- Utility Functions (Logs, Palette, VRAM) ---
...
static void doom_log_checkpoint(const char *where) {
    char buffer[192];
    int length = snprintf(buffer, sizeof(buffer), "%s tick=%d ms=%d\n", where, (unsigned int)doom_tick_count, (unsigned int)get_ms_clock());
    if (length > 0) doom_log_write(buffer, length);
}
...
static void release_key(unsigned char key) { queue_key(0, key); }
...
static void release_game_keys(void) {
    ...
    release_key(KEY_FIRE); release_key(KEY_USE); release_key(KEY_RSHIFT);
}

// --- DOOMGeneric Interface ---
void DG_Init(void) { install_doom_palette(); }
void DG_DrawFrame(void) {
    uint8_t *vram = bmp_vram();
    if (!vram || !DG_ScreenBuffer) return;
    if (palette_changed) install_doom_palette();
    for (int y = 0; y < DOOM_H; y++) {
        memcpy(vram + (DOOM_Y + y) * BMPPITCH + DOOM_X, (uint8_t *)DG_ScreenBuffer + y * DOOM_W, DOOM_W);
    }
}
...
int DG_GetKey(int *pressed, unsigned char *doom_key) {
    ...
    key_read = (key_read + 1) % KEYQUEUE_SIZE;
    *pressed = (event >> 8) & 1; *doom_key = event & 0xff;
    return 1;
}
...
static void cycle_owned_weapon(int direction) {
    ...
    int start = (player->pendingweapon != wp_nochange) ? (int)player->pendingweapon : (int)player->readyweapon;
    for (int step = 1; step < NUMWEAPONS; step++) {
        int candidate = (start + direction * step + NUMWEAPONS) % NUMWEAPONS;
        if (player->weaponowned[candidate]) { player->pendingweapon = (weapontype_t)candidate; return; }
    }
}

// --- 60D Button Mapping ---
#define BGMT_60D_UP     0x24
#define BGMT_60D_DOWN   0x28
#define BGMT_60D_LEFT   0x2a
#define BGMT_60D_RIGHT  0x26
#define BGMT_60D_UNPRESS_UDLR 0x2c
#define BGMT_60D_SET    0x04
#define BGMT_60D_SET_UP 0x05
#define BGMT_60D_WHEEL_L 0x02
#define BGMT_60D_WHEEL_R 0x03
#define BGMT_60D_MENU   0x06
#define BGMT_60D_INFO   0x07
#define BGMT_60D_PLAY   0x09
#define BGMT_60D_TRASH  0x0a
#define BGMT_60D_ZOOM_IN 0x0d
#define BGMT_60D_ZOOM_IN_UP 0x0e
...
static unsigned int doom60d_keypress_raw(unsigned int context) {
    ...
    switch (key) {
        case BGMT_60D_UP:    queue_key(1, KEY_UPARROW);    return 0;
        case BGMT_60D_DOWN:  queue_key(1, KEY_DOWNARROW);  return 0;
        case BGMT_60D_LEFT:  queue_key(1, KEY_LEFTARROW);  return 0;
        case BGMT_60D_RIGHT: queue_key(1, KEY_RIGHTARROW); return 0;
        case BGMT_60D_UNPRESS_UDLR: 
            release_key(KEY_UPARROW); release_key(KEY_DOWNARROW);
            release_key(KEY_LEFTARROW); release_key(KEY_RIGHTARROW);
            return 0;
        case BGMT_60D_SET:   queue_key(1, KEY_FIRE); return 0;
        case BGMT_60D_SET_UP: release_key(KEY_FIRE); return 0;
        case BGMT_60D_WHEEL_L: cycle_owned_weapon(-1); return 0;
        case BGMT_60D_WHEEL_R: cycle_owned_weapon(1);  return 0;
        case BGMT_60D_PLAY:  tap_key(KEY_USE); return 0;
        case BGMT_60D_INFO:  tap_key(KEY_TAB); return 0; // Map
        case BGMT_60D_MENU:  tap_key(KEY_ESCAPE); return 0;
        case BGMT_60D_ZOOM_IN: queue_key(1, KEY_RSHIFT); return 0; // Run
        case BGMT_60D_ZOOM_IN_UP: release_key(KEY_RSHIFT); return 0;
        default: return 1;
    }
}
...
static void doom60d_task(void *arg) {
    // Logic for WAD loading and doomgeneric_Create/Tick (from original file)
    // ...
    doom_running = 1;
    char *argv[] = { "doom", "-iwad", doom_requested_wad_path, 0 };
    doomgeneric_Create(3, argv);
    while (doom_running && !ml_shutdown_requested) {
        doomgeneric_Tick();
    }
...
static struct menu_entry doom60d_menu[] = {
    { .name = "Doom 60D", .select = doom60d_start, .help = "Run Doom on EOS 60D" }
};
